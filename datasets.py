...
class QuoteCSE_Dataset(Dataset):
    def __init__(self, args, title_texts, body_texts, pos_idx, neg_idx, neg_cnt, max_seq, neg_max_seq):
        ...
        for idx in tqdm(range(len(title_texts))):
            ...
            neg = -1 * np.ones(self.neg_max_seq)
            neg[:len(neg_idx[idx])] = neg_idx[idx]

            self.title.append(title_input)
            self.body.append(body_input)
            self.pos_idx.append(pos_idx[idx])
            self.neg_idx.append(neg)
            self.neg_cnt.append(neg_cnt[idx])
            
    def __len__(self):
        return len(self.title)
    
    def __getitem__(self, idx):
        return self.title[idx], self.body[idx], self.body_len[idx], torch.tensor(self.pos_idx[idx], dtype=torch.long), torch.tensor(self.neg_idx[idx], dtype=torch.long), torch.tensor(self.neg_cnt[idx], dtype=torch.long)
...
def create_data_loader(args, df, shuffle, drop_last):
    cd = QuoteCSE_Dataset(
        ...
        neg_cnt=df.neg_cnt.to_numpy(),
        max_seq = max(df.sentence_quotes.apply(len).values),
        neg_max_seq = max(df.neg_idx.apply(len).values),
    )
